...
class Blockchain {
  ...
  async filterUtxosByTokenAndBch (utxos) {
    try {
      ...
      return {
        tokenUTXOs: utxosWithTokenDetails.slpUtxos[0].utxos.filter(utxo => utxo.isValid),
        bchUTXOs: utxosWithTokenDetails.slpUtxos[0].utxos.filter(utxo => utxo.isValid === false)
      }
    } catch (e) {
